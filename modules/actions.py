...
def display_books(win_name,xc,yc,h,w):
        genre = pymsgbox.prompt(text="Which type of books you want?(Economics,Programming, etc.)",title="Receiptionist")
        genre = genre.lower()
        all_entries = os.listdir(folder_path)
        files_only = [f for f in all_entries if os.path.isfile(os.path.join(folder_path, f))]
        fl=0
        if genre+"_books.txt" in files_only:
